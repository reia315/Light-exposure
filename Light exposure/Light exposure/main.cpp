...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hInstancePrev, LPSTR pCmdLine, int nCmdShow)
{
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			DirectX::XMVECTOR cameraPos = { 0.0f, 0.0f, -5.0f };
			DirectX::XMVECTOR lightPos = { 0.0f, 0.0f, -100.0f };
			...
			ConstantBuffer cbf;
			cbf.WorldMatrix = world;
			cbf.ViewMatrix = view;
			cbf.ProjectionMatrix = projection;
			cbf.WorldViewProjMatrix = world * view * projection;
			cbf.CameraPosition = cameraPos;
			cbf.LightPosition = lightPos;
			...
			pDeviceContext->PSSetShaderResources(0, 1, &pTextureShaderResourceView);
